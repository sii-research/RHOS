...
def count_task_states():
    """返回 (total, done_count, locked_count, pending_count)"""
    if not Path(TASK_FILE).exists():
        return 0, 0, 0, 0

    # 总任务数 = 文件行数（含空行/无效行，但通常我们生成时已过滤）
    total = sum(1 for line in open(TASK_FILE) if line.strip())  # 更健壮：跳过空行

    done_count = len(list(LOCKS_DIR.glob(f"*{DONE_SUFFIX}")))
    locked_count = len(list(LOCKS_DIR.glob(f"*{LOCK_SUFFIX}")))

    pending_count = total - done_count - locked_count
    pending_count = max(0, pending_count)  # 防文件变动导致负数
...
def main():
    ...
    print(f"🔍 监控任务文件: {TASK_FILE}")
    print(f"📁 锁目录: {LOCKS_DIR}")

    while True:
        total, done, locked, pending = count_task_states()

        print(f"\n📊 任务状态 | 总: {total} | ✅已完成: {done} | 🟡运行中: {locked} | ⏳待领取: {pending}")
        ...
        # ✅ 有 pending 任务 → 启动 workers 抢占
        print(f"🔄 启动 {n_gpus} 个 worker (GPU 0 ~ {n_gpus-1}) 抢占 {pending} 个待办任务...\n")
