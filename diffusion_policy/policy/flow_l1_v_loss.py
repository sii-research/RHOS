...
class FlowUnetL1SampleHybridImagePolicy(BaseImagePolicy):
    def __init__(self, 
        ...
        # num_inference_steps is not used when NFE < 1
        self.num_inference_steps = num_inference_steps
        self.first_t = self.num_inference_steps
    ...
    def conditional_sample(self, 
        ...
        # Use standard Flow Matching inference
        if self.num_inference_steps > 1:
            ...
            # 2. first step: x_0 -> x_0.5
            a_pred = model(x_t, t, 
                local_cond=local_cond, global_cond=global_cond)
            
            # 3. compute previous image: x_t -> x_{t + dt}
            v_t = (a_pred - x_t)/(1-t)
            x_t = x_t + dt*v_t
            t = t + dt
            
            # 4. second step: x_0.5 -> x_1
            a_pred = model(x_t, t, 
                local_cond=local_cond, global_cond=global_cond)   
