...
def has_successful_ckpt(ckpt_dir, threshold=0.5):
    ...
    for ckpt_path in ckpt_dir.glob("*.ckpt"):
        ...
        if match:
            ...
            if score > threshold and epoch > 180:
                return True
    return False
...
def count_total_tasks():
    if not Path(TASK_FILE).exists():
        print(f"❌ 任务文件 {TASK_FILE} 不存在，请先运行 generate_cmd.py")
        return 0
    with open(TASK_FILE) as f:
        return sum(1 for _ in f)

def main():
    parser = argparse.ArgumentParser(description="使用单个训练 worker")
    parser.add_argument("use_gpu", type=int, help="使用的 GPU 序号（0,1,2,3）")
