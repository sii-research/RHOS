...
class L1UnetHybridImagePolicy(BaseImagePolicy):
    def __init__(self, 
            shape_meta: dict,
            horizon, 
            n_action_steps, 
        ...
        self.model = model
        self.embed = nn.Parameter(torch.randn(horizon, action_dim))
        ...
        print("L1 net params: %e" % sum(p.numel() for p in self.model.parameters()))
    ...
    def conditional_sample(self, 
        ...
        model = self.model
        embed = self.embed.unsqueeze(0).expand(condition_data.shape[0], *self.embed.shape)
    
        t = torch.ones(1, device=condition_data.device)

        # predict model output
        trajectory = model(embed, t, 
    ...
    def compute_loss(self, batch):
        ...
        embed = self.embed.unsqueeze(0).expand_as(trajectory)
    
        t = torch.ones(1, device=trajectory.device)

        # 2. predict model output
        pred_trajectory = self.model(embed, t, 
                local_cond=local_cond, global_cond=global_cond)
        ...
        loss = F.l1_loss(pred_trajectory, trajectory, reduction='none')
