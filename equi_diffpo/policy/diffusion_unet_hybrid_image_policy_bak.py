...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        naction_refine = self.model(naction_pred, refine_tag, local_cond=None, global_cond=global_cond)
    ...
    def compute_loss(self, batch, weight):
        # normalize input
        ...
        refine_action = self.model(pred_nactions, refine_tag, local_cond=local_cond, global_cond=global_cond)
        refine_loss = F.l1_loss(refine_action, nactions, )
        # deri_loss = F.mse_loss(refine_action[:,1:]-refine_action[:,:-1], nactions[:,1:]-nactions[:,:-1], )
        ...
        loss = diff_loss + refine_loss #+ deri_loss
        return loss
