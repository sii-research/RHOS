...
class DiffusionEquiUNetCNNEncPolicy(BaseImagePolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor], is_refine=True) -> Dict[str, torch.Tensor]:
        """
        ...
        # run sampling
        nsample = self.conditional_coarse_sample(
            trajectory,
            cond_data,
            cond_mask,
            local_cond=None,
        ...
        refine_tag = -torch.ones(B,device=device)
        naction_refine = self.diff(naction_pred, refine_tag, local_cond=None, global_cond=global_cond)
        action_refine = self.normalizer["action"].unnormalize(naction_refine)
        action_pred = action_refine[...,:Da]
        
        # # run sampling(initial)
    ...
    def compute_loss(self, batch, if_val=False):
        # normalize input
        ...
        refine_tag = -torch.ones_like(timesteps)
        
        sample_noise = torch.randn(trajectory.shape, device=trajectory.device)
        if not if_val:
            sample_noise.requires_grad_(True)
            guide_loss = ((sample_noise - nactions) ** 2).sum(dim=2).mean()
            guide_grad = torch.autograd.grad(guide_loss, sample_noise)[0]
            # gradient guidance
            sample_noise = sample_noise - 5*guide_grad
        ...
        with torch.no_grad():
            ...
            pred_nactions = nsample[..., :self.action_dim]

        refine_action = self.diff(pred_nactions, refine_tag, local_cond=None, global_cond=global_cond)
        refine_loss = F.l1_loss(refine_action, nactions, )
        deri_loss = F.l1_loss(refine_action[:,1:]-refine_action[:,:-1], nactions[:,1:]-nactions[:,:-1], )
        ...
        diff_loss = diff_loss * loss_mask.type(diff_loss.dtype)
        diff_loss = reduce(diff_loss, 'b ... -> b (...)', 'mean')
        diff_loss = diff_loss.mean()
        loss = diff_loss + 0.1*refine_loss + 0.1*deri_loss
        return loss